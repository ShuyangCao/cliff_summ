...
def collate(
    ...
    src_select_index = torch.LongTensor([i for i, s in enumerate(samples) for _ in s['pos_target']])

    contrast_prev_output_tokens = data_utils.collate_tokens(
        [tgt for s in samples for tgt in s['pos_target']],
        pad_idx,
        eos_idx,
        left_pad_target,
        True,
    ...
    if samples[0].get("pos_ne", None) is not None:
        ...
        valid_contrast = torch.ones((contrast_ne_tokens.size(0), contrast_ne_tokens.size(0)), dtype=torch.bool)
        ...
        valid_contrast = torch.ones((contrast_prev_output_tokens.size(0), contrast_prev_output_tokens.size(0)), dtype=torch.bool)
