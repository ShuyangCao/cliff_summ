...
class LanguagePairDataset(FairseqDataset):
    ...
    def __init__(
        ...
        self.pos_ids = list(sorted(pos_samples.keys()))
        self.pos_shuffle = {id: np.random.permutation(len(pos_samples_i)) for id, pos_samples_i in pos_samples.items()}
    ...
    def __getitem__(self, index):
        ...
        pos_tgt_items = self.pos_samples.get(index, [])
        assert pos_tgt_items

        pos_ne_items = [torch.tensor(
            [1] * len(pos_tgt_item) + [0],
            dtype=torch.long
        ) for pos_tgt_item in pos_tgt_items]

        pos_tgt_items = [torch.tensor(
            [self.tgt_dict.index(token) for token in pos_tgt_item] + [self.tgt_dict.eos()],
            dtype=torch.long
        ) for pos_tgt_item in pos_tgt_items]
        ...
        if neg_tgt_items:
            if len(self.neg_shuffle[index]) <= self.max_neg_samples:
                neg_ne_items = [torch.tensor(
                    [1] * len(neg_tgt_item) + [0],
                    dtype=torch.long
                ...
                neg_ne_items = [torch.tensor(
                    [1] * len(neg_tgt_items[self.neg_shuffle[index][i % len(self.neg_shuffle[index])]]) + [0],
                    dtype=torch.long
                )
