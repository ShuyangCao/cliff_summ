...
class ReGenerator(nn.Module):
    ...
    def _generate(
        ...
        # NEW: get text
        ent_prefix_tokens = net_input["prev_output_tokens"]
        ent_prefix_tokens = ent_prefix_tokens[:, :800]
        ...
        # NEW: prepare entity types and prefix tokens
        ent_prefix_tokens = ent_prefix_tokens.index_select(0, new_order)
        ...
        # NEW: use the last token in the prefix
        tokens[:, 0] = ent_prefix_tokens[:, -1]
        ent_prefix_tokens = ent_prefix_tokens[:, :-1]
        ...
        for step in range(max_len + 1):  # one extra step for EOS marker
            ...
            if reorder_state is not None:
                ...
                ent_prefix_tokens = ent_prefix_tokens.index_select(0, reorder_state)

            if step == 0:
                # NEW: init incremental state with prefix
                lprobs, avg_attn_scores = self.model.forward_decoder(
                    torch.cat([ent_prefix_tokens, tokens[:, : step + 1]], dim=-1),
                    encoder_outs,
                    incremental_states,
                    self.temperature,
                    True
            ...
            lprobs[:, self.unk] = -math.inf  # apply unk penalty

            # print(lprobs)
            # print(lprobs[:, self.pad])

            if ent_prefix_tokens.size(1) + step > 800:
    ...
    def forward_decoder(
        ...
        for i, model in enumerate(self.models):
            ...
            if self.has_incremental_states():
                ...
                decoder_out = model.decoder.forward(tokens, encoder_out=encoder_out, prefix=True, prefix_init=init)
